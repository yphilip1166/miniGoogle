...
public class MapBolt implements IRichBolt {
    ...
    public void prepare(Map<String,String> stormConf, 
        ...
        neededVotesToComplete = (intWorker - 1)*intMap*intFileSpout + intFileSpout;
    }
    ...
    public synchronized void execute(Tuple input) {
    	if (!input.isEndOfStream()) {
	        ...
	        mapJob.map(key, value, this.collector);
	        // @piazza 1009
	        WorkerServerInfo.incrementKeysWritten();
	        // @piazza 1014
	        WorkerServerInfo.setStatus("MAPPING");
    	} else if (input.isEndOfStream()) {
    		// emit with EOS
    		if (--neededVotesToComplete == 0){
    			collector.emitEndOfStream();
    			// @piazza 1014
    			WorkerServerInfo.setStatus("WAITING");
    			// @piazza 1009
    			WorkerServerInfo.resetCounter();
    		}
    	}
    }
