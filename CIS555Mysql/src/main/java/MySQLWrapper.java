...
public class MySQLWrapper implements IndexingSQL{
    ...
    public static int RESULT_NUMBER = 10;
    public static float WORD_NEIGHBOR_WEIGHT = 1.0f;
    ...
    public List<IndexingItem> getWordUrls(String word){
        try {
            preparedStatement = connect.prepareStatement("SELECT *, ((tf+headingScore+positionScore)*((capitalPercent+titlePercent+linkPercent " +
                    "+emphasisPercent+metaPercent)+" +String.valueOf(TF_WEIGHT)+ ")) as scoreSum from Indexing where word = ? ORDER BY scoreSum DESC LIMIT "+String.valueOf(TOPK_PERWORD));
            preparedStatement.setString(1, word);
            resultSet = preparedStatement.executeQuery();
